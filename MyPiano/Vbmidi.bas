...
Public Sub MidiOpen()
    rc = midiOutOpen(hMidi, DeviceID, 0, 0, 0)
    If rc <> 0 Then MsgBox "Open MIDI Out failed"
End Sub

Public Sub MidiClose()
    rc = midiOutClose(hMidi)
    If rc <> 0 Then MsgBox "Close MIDI Out failed"
End Sub
...
Public Sub StartNote(ByVal Channel As Integer, ByVal Index As Integer, ByVal Volume As Integer)
    ...
    rc = midiOutShortMsg(hMidi, midimsg)
End Sub
